# Hi, I'm Raseef Azeez 👋

### 🚀 DevOps Engineer | AWS | Kubernetes | Terraform | CI/CD

Cloud Engineer with 5+ years of AWS experience, now focused on DevOps — building **automated, scalable, and resilient cloud-native systems**.

I enjoy working on systems that don’t just run — but **recover, scale, and adapt**.

💡 I believe in architecting simplicity in the cloud — understanding systems by asking one “why” at a time.

---

## 🌐 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge\&logo=linkedin)](https://www.linkedin.com/in/raseef-azeez)
[![Portfolio](https://img.shields.io/badge/Portfolio-Website-black?style=for-the-badge\&logo=google-chrome)](https://d3svccsjj104ji.cloudfront.net)

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=radical&hide_border=true" height="150"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=radical&hide_border=true" height="150"/>
</p>

---

## 🧠 About Me

* ☁️ 5+ years building on AWS
* ⚙️ Transitioning into full-scale DevOps engineering
* 🔁 Strong focus on **automation, recovery & system reliability**
* 🧩 Passionate about solving **real-world operational problems through DevOps**

---

## 🧠 Featured Project: RaaS (Reboot as a Service)

A self-service platform I built to **reduce operational overhead by automating EC2 lifecycle actions** — inspired by real-world support bottlenecks.

🔗 https://d1l62wn91r29l3.cloudfront.net

---

### 🏗 Architecture Overview

![RaaS Architecture](./architecture.png)

---

### 🔍 Flow Summary

1. User accesses the platform via **CloudFront-hosted frontend**
2. Authenticates using **Amazon Cognito (OAuth2 / JWT)**
3. API Gateway validates JWT via **Authorizer**
4. Lambda processes request and interacts with **EC2 lifecycle APIs**
5. Authorization enforced using **tag-based access control (OwnerGroup)**
6. Infrastructure managed via **Terraform + GitHub Actions (OIDC, no static credentials)**

👉 Designed with a focus on **security, automation, and operational efficiency**

---

### 🔹 Infrastructure & Automation

* Fully modular Terraform setup (multi-environment: dev/prod)
* GitHub Actions CI/CD with OIDC (no static credentials)
* Reusable Terraform modules

---

### 🔹 What This Reflects About Me

* I solve **operational inefficiencies through automation**
* I design systems with **security (identity-first) and scalability in mind**
* I build with a **production mindset, not just demos**

👉 Deep Dive: **[https://raseefazeez.hashnode.dev/raas-reboot-as-a-service]**

---

## 🌐 Personal Website

My cloud-hosted portfolio — built to practice real-world architecture patterns

🔗 https://d3svccsjj104ji.cloudfront.net

---

### 🏗 Architecture Overview

![Portfolio Architecture](./portfolio-architecture.png)

---

### 🔹 Architecture Highlights

* S3 (Static Hosting)
* CloudFront (CDN)
* API Gateway + Lambda (Backend)
* DynamoDB (Storage)
* CI/CD pipeline

👉 Architecture Deep Dive: **[https://raseefazeez.hashnode.dev/how-i-built-my-cloud-portfolio-using-aws-and-github-actions]**

---

## 📝 Deep Dive: How I Build Systems

I document how I design, break, and improve systems to reflect real-world DevOps practices.

* Serverless architecture design
* Infrastructure as Code (Terraform / CloudFormation)
* Secure CI/CD (OIDC-based pipelines)
* Debugging real-world production issues
* Designing for **failure, recovery, and scalability**

> Certifications build knowledge, but real projects build understanding.

---

## 🛠 Tech Stack

![AWS](https://img.shields.io/badge/AWS-orange?style=for-the-badge\&logo=amazon-aws)
![Terraform](https://img.shields.io/badge/Terraform-purple?style=for-the-badge\&logo=terraform)
![Kubernetes](https://img.shields.io/badge/Kubernetes-blue?style=for-the-badge\&logo=kubernetes)
![Docker](https://img.shields.io/badge/Docker-blue?style=for-the-badge\&logo=docker)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-black?style=for-the-badge\&logo=github-actions)

---

## 📂 Featured Projects

### 🔹 AWS Serverless Web Platform

* S3 + CloudFront hosting
* API Gateway + Lambda backend
* DynamoDB integration
* CI/CD automation

---

### 🔹 Kubernetes Microservices Platform *(In Progress)*

* Deployments, Services, Ingress
* ConfigMaps & Secrets
* Scalable architecture

---

### 🔹 CI/CD Pipeline Automation

* Multi-stage GitHub Actions workflows
* Docker build & push
* Automated deployments

---

### 🔹 Terraform Infrastructure Modules

* VPC, IAM, compute resources
* Modular and reusable design

---

## 📈 Current Focus

* Containerizing **RaaS** → Kubernetes
* Running locally using **Minikube / Kind**
* Exploring cloud deployment (Oracle Cloud Free Tier)
* Observability (Prometheus, Grafana, alerting)

---

## ⚡ Beyond Tech

I believe good engineers don’t just build systems — they **anticipate failure, reduce friction, and design for resilience**.


